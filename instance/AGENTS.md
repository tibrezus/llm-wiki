...
#### How C4 levels are assigned

**This is your job as the LLM** — the core value-add. The graph gives you
ranked symbols and their reference edges. You read `<project>.map.txt`, drill
the `.scip` for specific neighborhoods when needed, and decide which clusters
are Context vs Container vs Component vs Code. The ranking (PageRank) tells you
what is load-bearing (high rank → prominent in the diagram) versus incidental.
...
2. **Read the map** — orient using `<project>.map.txt`.
3. **Re-derive C4 levels** — assign symbols/clusters to levels.
4. **Update D2 figures** in the pages that explain each topic, preserving the
   C4 nesting and cross-board links.
5. **Update `index.md`** and **append to `log.md`**; list the graph file in
   `sources:`.
